...
to a native-1-bit model whose weights we leave untouched. A complementary
algorithm--architecture line treats outliers as a hardware problem:
MicroScopiQ~\cite{ramachandran2025microscopiq} keeps outliers at higher precision and
prunes a fraction of low-importance weights to ``fund'' the extra outlier bits on a
custom multi-precision accelerator---reaching the same conclusion from the systems
side that outlier \emph{handling}, not raw bit-width, is the design axis.
...
\url{https://github.com/kksante/bitnet-int8-rs} (commit \texttt{b7c9846}); model
...
\bibitem{ramachandran2025microscopiq} A.~Ramachandran, S.~Kundu, T.~Krishna.
MicroScopiQ: Accelerating Foundational Models through Outlier-Aware Microscaling
Quantization. ISCA, 2025 (arXiv:2411.05282).
